 ...
 * その結果「音声を送れないProviderへ音声リクエストがルーティングされ、実行して初めて失敗する」
 * 状態だった（ADR-0039）。Routingがハードフィルタとして読める形にするには、
 * 突き合わせ可能な列挙である必要がある。
 *
 * 申告の置き場は**(Model, Capability)**である。ADR-0039ではSPI側にも申告口
 * （`CapabilityConstraints.supportedInputModalities`）を作ったが、そちらは
 * (Provider, Capability)の粒度でRoutingの判断には使えず、消費者が現れないままだったため
 * ADR-0042で削除した。Adapterから申告させるなら`DiscoveredModel`（Modelごと）へ足すこと。
 *
